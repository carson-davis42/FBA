 ...
 *
 *  On my honor, Carson Davis, this programming assignment is my own work
 *  and I have not provided this code to any other student.
 *
 *  Number of slip days used: 2
 *
 *  UTEID: csd2424
 *  email address: carsonsd@utexas.edu
 *  TA name: Lauren
 */
...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        Queue<Path> q = prepForDijkstra(startName);
        List<Vertex> visited = new ArrayList<>();
        //Loop through each path created until none remain unchecked
        while (!q.isEmpty()) {
            Path current = q.remove();
            Vertex curVert = current.dest;
            if (!visited.contains(curVert)) {
                //This is the quickest route to curVert, only check its edges once for efficiency
                visited.add(curVert);
                //Loop through edges and check each neighbor
                for (Edge e : curVert.adjacent) {
                    Vertex neighbor = e.dest;
                    double curCost = curVert.weightedCostFromStartVertex + e.cost;
                    if (curCost < neighbor.weightedCostFromStartVertex) {
                        //Lowest cost to get to neighbor
                        neighbor.weightedCostFromStartVertex = curCost;
                        neighbor.numEdgesFromStartVertex = curVert.numEdgesFromStartVertex + 1;
                        neighbor.prev = curVert;
                        //Make a new path with everything from current but we add neighbor
                        //and its cost
                        Path newPath = new Path();
                        newPath.verticesInPath.addAll(current.verticesInPath);
                        newPath.add(neighbor);
                        newPath.dest = neighbor;
                        newPath.weightedCostOfPath += neighbor.weightedCostFromStartVertex;
                        q.add(newPath);
                    }
                }
            }
    ...
    /**
     * Gets the queue and vertices ready for dijkstra, resets all values
     * @param startName pre: startName != null
     * @return the priority queue needed for dijkstra()
     */
    private Queue<Path> prepForDijkstra(String startName) {
        currentStartVertexName = startName;
        clearAll();
        Vertex startVert = vertices.get(startName);
        Path start = new Path(startVert, 0.0);
        startVert.weightedCostFromStartVertex = 0;
        startVert.numEdgesFromStartVertex = 0;
        Queue<Path> result = new PriorityQueue<>();
        result.add(start);
        return result;
    }
    ...
    public void findAllPaths(boolean weighted) {
        // TODO CS314 Students, complete this method.
        longest = new Path();
        longest.weightedCostOfPath = Integer.MIN_VALUE;
        //Loop through each vertex name in the graph
        for (String curStartName : vertices.keySet()) {
            Vertex curStartVert = vertices.get(curStartName);
            //Reset all values we need to calculate
            curStartVert.numVertexConnected = 0;
            curStartVert.totalUnweightedPathLength = 0;
            curStartVert.totalWeightedPathLength = 0;
            if(weighted) {
                dijkstra(curStartName);
            }
            else {
                findUnweightedShortestPath(curStartName);
            }
            //Loop through each name in the graph again to find the path from current vertex to
            //every other vertex if it exists
            for (String temp : vertices.keySet()) {
                Vertex tempVert = vertices.get(temp);
                if (tempVert.weightedCostFromStartVertex < INFINITY &&
                        tempVert.weightedCostFromStartVertex != 0) {
                    //A path exists and they are not the same vertex
                    curStartVert.numVertexConnected++;
                    Path tempPath = getPath(temp);
                    curStartVert.totalUnweightedPathLength += tempPath.verticesInPath.size() - 1;
                    curStartVert.totalWeightedPathLength += tempPath.weightedCost();
                    if (tempPath.weightedCostOfPath > longest.weightedCostOfPath) {
                        longest = tempPath;
                    }
                }
            }
