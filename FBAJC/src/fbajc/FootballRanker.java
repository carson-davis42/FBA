...
public class FootballRanker {
     ...
     */
    public FootballRanker(String gameResults) {
        if (gameResults == null) {
     ...
     * rounded to 1 decimal place.
     */
    public TreeSet<AllPathsInfo> doWeightedAndWinPercentAdjusted(boolean showResults) {
        if (showResults) {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        //Create a string list with the predicted rankings in order
        for (AllPathsInfo pathInfo : paths) {
            ranks.add(pathInfo.getName());
        }
        double totalDiff = 0;
        double tempDiff;
        int numSpaces = 0;
        //Find the number of spaces needed in the output depending on the longest name
        if (showResults) {
            int longestName = 0;
            ...
            final int DISTANCE_TO_DASH = 2;
            numSpaces = longestName + DISTANCE_TO_DASH;
        }
        //Iterate through each team in the ap poll and calculate
        for (int rank = 0; rank < humanRanks.size(); rank++) {
            if (ranks.contains(humanRanks.get(rank))) {
                tempDiff = Math.abs(rank - ranks.indexOf(humanRanks.get(rank)));
            }
            else {
                //Assign predicted rank as one more than the size of the predicted ranks
                tempDiff = Math.abs(rank - ranks.size());
            }
            tempDiff = tempDiff * tempDiff;
            totalDiff += tempDiff;
            printTeam(showResults, humanRanks, ranks, rank, numSpaces);
        }
        totalDiff = Math.sqrt(totalDiff / ((double) humanRanks.size()));
        if (showResults) {
            System.out.println("Root Mean Square Error: " + ourFormatter.format(totalDiff));
        }
        DecimalFormat df = new DecimalFormat("0.0");
        return Double.parseDouble(df.format(totalDiff));
    }

    /**
     * method to print a line of team info if showResults is true in printRootMeanSquareError
     *
     * @param showResults pre: none
     * @param humanRanks  pre: humanRanks != null
     * @param ranks       pre: ranks != null
     * @param rank        pre: none
     * @param numSpaces   pre: none
     */
    private void printTeam(boolean showResults, List<String> humanRanks, List<String> ranks,
                           int rank, int numSpaces) {
        if (showResults) {
            System.out.print(humanRanks.get(rank));
            for (int spaces = humanRanks.get(rank).length(); spaces < numSpaces; spaces++) {
                System.out.print(" ");
            }
            System.out.println("- actual rank: " + (rank + 1) + " predicted rank: " +
                    (ranks.indexOf(humanRanks.get(rank)) + 1));
        }
    }
